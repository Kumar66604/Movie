...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton("Cʜᴀɴɴᴇʟ 🔈", url="https://t.me/+lEglTmkNUmgzY2Y1")
            ],[      
            InlineKeyboardButton("Hᴇʟᴩ 🕸️", callback_data="help"),
            InlineKeyboardButton("Aʙᴏᴜᴛ ✨", callback_data="about")
        ]]
        m = await message.reply_sticker("CAACAgUAAxkBAAIDGWVYXpG91pSi-wAB5frAUr74NM4lmgACZwIAAoZc8VSxqaPQZ6_jNTME") 
        await asyncio.sleep(2)
        await message.reply_video(video=random.choice(PICS), caption=START_MESSAGE.format(user=message.from_user.mention, bot=client.mention), reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
